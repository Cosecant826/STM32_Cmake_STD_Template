...
static struct {
    ...
    union {
        ...
        struct {
            unsigned char IACCVIOL : 1; // Instruction access violation非法访问指令
    ...
    union {
        ...
        struct {
            ...
            unsigned int VECTBL : 1; // Indicates hard fault is caused by failed vector fetch中断向量表读取异常
            unsigned int UnusedBits2 : 28;
            unsigned int FORCED : 1;   // Indicates hard fault is taken because of bus fault/memory management fault/usage fault其他Fault触发的异常
